...
void FDTD::FDTD::shifted_field_update(const double t)
{
  ...
  for (double time = 0.0; time < t; time += dt) // ѕќѕ–ј¬»“№ Ќј time += E_dt
  {
    for (i = 0ull; i < Nx; ++i)
      for (j = 0ull; j < Ny; ++j)
        for (k = 0ull; k < Nz; ++k)
...
void FDTD::FDTD::shifted_field_update(const uint64_t t)
{
  ...
  // ѕока что предполагаетс€, что Ny кратно количеству процессов size! “акже пока что рассматриваетс€ MPI дл€ двумерного случа€!
